...
class SharedState {
  ...
  nextTurnNumber;
  checkWords;
  constructor(settings, gameId = settings.gameId ?? `game-${Date.now()}`, board = new Board(...settings.boardLayout), tilesState = makeTilesState(settings), nextTurnNumber2 = toTurnNumber(1), checkWords = makeDictionary(settings.dictionaryType, settings.dictionarySettings)) {
...
class GameState extends EventTarget {
  ...
  get turnUrlParams() {
    ...
    if (turnHistory.length) {
      ...
      const flatTurnParams = turnParams.map((p) => [...p]).flat();
